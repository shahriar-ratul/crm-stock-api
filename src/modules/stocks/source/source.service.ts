...
export class SourceService {
  ...
  async create(createDto: CreateCategoryDto) {
    const check = await this._prisma.source.findFirst({
      where: {
        OR: [
          { name: createDto.name },
        ],
    ...
    if (check) {
      throw new HttpException('source already exists', HttpStatus.BAD_REQUEST);
    }

    const item = await this._prisma.source.create({
      data: {
        name: createDto.name,
      },
    });
  ...
  // getAllItems
  async getAllItems() {
    const items = await this._prisma.source.findMany({
      orderBy: {
        name: 'asc',
      },
    });
